...
class Player {
  ...
  getItem(n) {
    let applicable = this.inventory.filter((e) => this.level >= e.minLevel && e.name === n);
...
function buttonClicked() {
  ...
  if (m = input.match(/move (\w+)/)) {
    ...
    for(let item of jeff.inventory.filter((e)=>jeff.level >= e.minLevel)) {
      Library.print("> "+item.name);
    }

  } else if (m = input.match(/use (\w+)/)) {
    let item = m[1];
    item = jeff.getItem(item);
